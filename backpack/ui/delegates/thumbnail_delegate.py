...
# ── Per-extension icons ─────────────────────────────────────────────────────
# Files with no image preview (scene/project files) show a branded icon instead
# of a generic glyph. Icons live in ui/resources/icons/.
...
_ext_icon_cache: dict[str, "QPixmap | None"] = {}


def _ext_icon(ext: str) -> "QPixmap | None":
    """Return a cached branded icon for *ext* (e.g. '.hip'), or None."""
    if ext in _ext_icon_cache:
        return _ext_icon_cache[ext]
    pm = None
    fn = _EXT_ICON_FILES.get(ext)
    if fn:
        path = _ICON_DIR / fn
        if path.exists():
            loaded = QPixmap(str(path))
            pm = loaded if not loaded.isNull() else None
    _ext_icon_cache[ext] = pm
    return pm
...
class ThumbnailDelegate(QStyledItemDelegate):
    ...
    @staticmethod
    def _draw_ext_icon(painter: QPainter, rect: QRect, index, frac: float = 0.62) -> bool:
        """Draw a branded per-extension icon centred in *rect*. Returns True if
        an icon was drawn (caller then skips the generic glyph)."""
        pm = _ext_icon(_name_ext(index.data(Qt.DisplayRole) or ""))
        if pm is None or pm.isNull():
            return False
        side = max(12, int(min(rect.width(), rect.height()) * frac))
        scaled = pm.scaled(side, side, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def _paint_row(self, painter: QPainter, option, index, compact: bool):
        ...
        # Indent items that live in subfolders below the selected folder.
        depth = index.data(DEPTH_ROLE) or 0
        left = rect.x() + 8 + min(depth, 5) * 14
        ...
        if not compact:
            ...
            if pix and not pix.isNull():
                ...
                yo = (pix.height() - tr.height()) // 2
                painter.drawPixmap(tr.topLeft(), pix, QRect(xo, yo, tr.width(), tr.height()))
            elif not self._draw_ext_icon(painter, tr, index, frac=0.8):
                painter.fillRect(tr, _C_SURFACE_PH)
                at = index.data(Qt.UserRole + 2) or "?"
                painter.setFont(QFont("Segoe UI Symbol", max(12, sq // 2), QFont.Light))
                painter.setPen(QColor(self._ROW_COLS.get(at, "#2a2d35")))
                painter.drawText(tr, Qt.AlignCenter, self._ROW_SYMS.get(at, "□"))
            painter.restore()
            left = tr.right() + 10
        ...
        # Date + size columns — list view only (data is only stat'd there).
        if not compact:
