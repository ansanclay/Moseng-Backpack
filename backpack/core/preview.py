...
# PIL is imported lazily inside generate_preview(); the scanner imports this
# module only for the path helpers, so this keeps PIL off the startup path.
...
def sync_previews(
    ...
    from backpack.core.scanner import (_collect_material_dirs, _collect_model_asset_dirs,
                                        _scandir_entries, _entry_is_dir, _entry_is_file,
                                        _entry_ext)
    ...
    def _collect_leaf_images(folder: Path):
        ...
        for entry in _scandir_entries(folder):
            ...
            if _entry_is_file(entry):
                if _entry_ext(entry) in _IMAGE_EXTS:
                    _files.append(Path(entry.path))
            elif _entry_is_dir(entry):
                for sub in _scandir_entries(entry.path):
                    if _entry_is_file(sub) and _entry_ext(sub) in _IMAGE_EXTS:
                        _files.append(Path(sub.path))
